...
+(NSString*)genWeixinShareUrl:(OSMessage*)msg to:(int)shareTo{
    ...
    if (!msg.multimediaType) {
        ...
        if ([msg isEmpty:@[@"image",@"link", @"file"] AndNotEmpty:@[@"title"]]) {
            ...
            //有链接。
            dic[@"description"]=msg.desc?msg.desc:msg.title;
            dic[@"mediaUrl"]=msg.link;
            dic[@"objectType"]=@"5";
            dic[@"thumbData"]=msg.thumbnail? [self dataWithImage:msg.thumbnail scale:CGSizeMake(100, 100)]:[self dataWithImage:msg.image];
            dic[@"title"] = shareTo == 1 ? (msg.desc?msg.desc:msg.title) : msg.title;
